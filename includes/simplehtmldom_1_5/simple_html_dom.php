...
class simple_html_dom_node {
    ...
    //@modified by bear@deepshiftlabs.com - added $except_tag parameter. Text inside $except_tag children tags will be skipped.
    // This is used when finding article text. See for example http://habrahabr.ru/company/mailru/blog/200394/ (russian)
    function text($except_tag = false)
    {
        if (isset($this->_[HDOM_INFO_INNER])) return $this->_[HDOM_INFO_INNER];
        ...
        if (!is_null($this->nodes))
        {
            foreach ($this->nodes as $n)
            {
                // added by bear@deepshiftlabs.com
                if ($except_tag && strcasecmp($n->tag, $except_tag)===0) {
                    continue;
                }
                // end of added by bear@deepshiftlabs.com
    ...
    function find($selector, $idx=null, $lowercase=false)
    {
        ...
        foreach ($found_keys as $k=>$v) {
            ...
            $temp = $this->dom->nodes[$k];
            $temp->nodetype=34343;
            $temp->nodetype=34343;
            // вот эта хрень является объектом и поэтому похоже возвращается по ссылке.
            //надо проеверить, ща
        }
