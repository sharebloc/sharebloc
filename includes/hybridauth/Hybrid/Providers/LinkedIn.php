	...
	*/
	function getUserProfile()
	{
		...
		if( isset( $response['success'] ) && $response['success'] === TRUE ){
            ...
            /* Added 30 Aug 2013 by bear@deepshiftlabs.com to fetch company and position data for LI */
			if( $data->{'positions'} && $data->{'positions'}->{'position'} ){
                $this->user->profile->li_position = (string) $data->{'positions'}->{'position'}->{'title'};

                if ($data->{'positions'}->{'position'}->{'company'}->{'name'}) {
                    $this->user->profile->li_company = (string) $data->{'positions'}->{'position'}->{'company'}->{'name'};
                }
			}
            /* END OF Added 30 Aug 2013 by bear@deepshiftlabs.com to fetch company and position data for LI */
	...
	*/
	function getUserContacts()
	{
		...
		foreach( $connections->person as $connection ) {
			...
			$uc->first_name = (string) $connection->{'first-name'};
			$uc->last_name = (string) $connection->{'last-name'};
